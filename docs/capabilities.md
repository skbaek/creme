...
| Luna reserve pseudo-subagent | ChatGPT-bundled `codex app-server` (override `CREME_LUNA_RESERVE_CODEX`); zero-token bucket read, pinned `gpt-reserve`, live and rollout attribution; session broker on a `0600` Unix socket with `LOCAL_PEERCRED` uid check | no default binary: `UNAVAILABLE` unless the override names a reviewed binary; broker uid check through `SO_PEERCRED` | refusal (exit `10`); an attribution failure (exit `12`) trips a tripwire that refuses every later run and stops every brokered session; without a peer-uid facility the broker relies on socket and directory modes |
