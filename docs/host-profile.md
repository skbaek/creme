...
`heavy_workers` as a ceiling, then admits a unit when its need plus the needs
already admitted fits live aggregate headroom less a fixed floor. An agent may
provide a larger conservative `--memory-gib` estimate for one operation; that dynamic
