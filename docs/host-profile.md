...
`MISSING` is supported on a host with no local findings. Invalid, empty,
non-UTF-8, NUL-containing, or oversized guidance fails the doctor check. The
64-KiB ceiling keeps this safety layer reviewable. `AGENTS.md` requires agents
to read valid local guidance before host-intensive work.
