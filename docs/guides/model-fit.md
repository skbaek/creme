...
| `route` | a route tag, then free detail: `claude-agent-tool`, `claude-session`; `codex-subagent`, `codex-session`, `luna-reserve-broker`, `luna-reserve-run`; `muse-worker`, `muse-session`. Detail names the harness, e.g. `luna-reserve-broker (Lean mode, Claude Opus master)` |
| `goal` | goal id, or `n/a` |
...
least three verified runs is marked `guides`. Under the grid, each populated
cell lists its observations, the failure modes and rework they recorded, and
cost observables (medians and ranges of output tokens, input including cache,
wall time, and turns). Quality comes first in every cell; cost is second and
carries the cautions above.
