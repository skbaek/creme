...
| Luna reserve pseudo-subagent | ChatGPT-bundled `codex app-server` (override `CREME_LUNA_RESERVE_CODEX`); zero-token bucket read, pinned `gpt-reserve`, live and rollout attribution; session broker on a `0600` Unix socket with `LOCAL_PEERCRED` uid check; opt-in Lean mode keeps only the tracked `lean-lsp-mcp`, one Lean session per broker, goal-scoped wind-down plus an in-target `lean`/`lake` process scan (`ps`, `lsof`) at every stop | no default binary: `UNAVAILABLE` unless the override names a reviewed binary; broker uid check through `SO_PEERCRED` | refusal (exit `10`); an attribution failure (exit `12`) trips a tripwire that refuses every later run and stops every brokered session; without a peer-uid facility the broker relies on socket and directory modes; a Lean session whose wind-down or scan is not `OK` ends `unclean` (exit `11`) |
...
the canonical Creme checkout. Codex installations may use the user-local
telemetry/reclamation bundle generated by `python3 -m creme host-wrappers`.
It contains argument-rejecting delegates plus a dedicated least-privilege
rules file. Preview is read-only; writing requires explicit executable and
rules directories, and replacement requires `--replace`. Codex must be fully
restarted after a rules change. Permission profiles control sandbox
